...
class AppUser extends Realm.Object {
  ...
  // Temporary method to check if we have collected
  // the device information from this user.
  // This is because older users created before the update
  // did not have the device information collected during sign up.
  get hasDeviceInfoTracked() {
    return Boolean(
      // Switch each to && once we need
      // to collect device model also.
      this.device_type || this.device_version || this.device_model,
    );
  }
...
class Contest extends Realm.Object {
...
class IntentionalWalk extends Realm.Object {
  get timeOfWalk() {
...
async function startWalk() {
  ...
  if (!walk) {
    ...
    if (!(await Notifications.isAuthorized())) {
      try {
        const settings = await Notifications.requestPermissions();
        console.log('requestPermissions', settings);
      } catch (error) {
        console.log('requestPermissions error', error);
      }
    }
    try {
      nextNotificationId = await Notifications.scheduleHourlyNotification(
...
async function addCurrentWalkListener(callback) {
  const realm = await open();
  const results = realm.objects('IntentionalWalk').filtered('end=null');
  function listener(newResults, changes) {
    callback(newResults.length > 0 ? newResults[0] : null);
  }
  results.addListener(listener);
  return () => results.removeListener(listener);
}
